...
class _RenderDataState extends State<RenderData> {
  ...
  bool _postureAccordingToExercise(Map<String, List<double>> poses) {
    ...
    if (excercise == 'squat') {
      if (squatUp) {
        return (poses['leftShoulder']?[1] ?? 0) < 320 &&
            (poses['leftShoulder']?[1] ?? 0) > 280 &&
            (poses['rightShoulder']?[1] ?? 0) < 320 &&
            (poses['rightShoulder']?[1] ?? 0) > 280 &&
            (poses['rightKnee']?[1] ?? 0) > 570 &&
            (poses['leftKnee']?[1] ?? 0) > 570;
      } else {
        return (poses['leftShoulder']?[1] ?? 0) > 475 &&
            (poses['rightShoulder']?[1] ?? 0) > 475;
      }
    }
    ...
    List<Widget> _renderKeypoints() {
      ...
      widget.data?.forEach((re) {
        var list = re?["keypoints"]?.values.map<Widget>((k) {
          ...
          if (widget.screenH != null &&
              widget.screenW != null &&
              widget.previewH != null &&
              widget.previewW != null) {
            ...
            if (x != null && y != null) {
              ...
              return Container(); // Return a default widget if x or y are null
            }
          } else {
            return Container(); // Return a default widget if screen dimensions are null
          }
        })?.toList() ?? [];
