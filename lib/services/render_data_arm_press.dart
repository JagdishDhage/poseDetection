...
class RenderDataArmPress extends StatefulWidget {
  ...
  RenderDataArmPress({
    required this.data, // Marking 'data' as required
...
class _RenderDataArmPressState extends State<RenderDataArmPress> {
  ...
  bool _postureAccordingToExercise(Map<String, List<double>> poses) {
    if (poses != null &&
        poses.containsKey('leftWrist') &&
        poses.containsKey('rightWrist') &&
        poses.containsKey('leftElbow') &&
        poses.containsKey('rightElbow') &&
        poses.containsKey('leftShoulder') &&
        poses.containsKey('rightShoulder') &&
        poses.containsKey('leftKnee') &&
        poses.containsKey('rightKnee')) {
      setState(() {
        wristLX = poses['leftWrist']?[0] ?? 0;
        wristLY = poses['leftWrist']?[1] ?? 0;
        wristRX = poses['rightWrist']?[0] ?? 0;
        wristRY = poses['rightWrist']?[1] ?? 0;
        elbowLX = poses['leftElbow']?[0] ?? 0;
        elbowRX = poses['rightElbow']?[0] ?? 0;

        shoulderLY = poses['leftShoulder']?[1] ?? 0;
        shoulderRY = poses['rightShoulder']?[1] ?? 0;
        kneeLY = poses['leftKnee']?[1] ?? 0;
        kneeRY = poses['rightKnee']?[1] ?? 0;
